    ...
    if (n > 0) {
        ...
        for (i = 0; i < n; ++i) {
            if (sd_is_socket_inet(SD_LISTEN_FDS_START + i, AF_INET, SOCK_STREAM, 1, port) > 0) {
                fd = SD_LISTEN_FDS_START + i;
                activated = true;
                pa_log_info("socket(): Found socket activation socket for ipv4 in port '%d' \\o/", port);
                break;
            }
        }
    }
    ...
    if (n > 0) {
        ...
        for (i = 0; i < n; ++i) {
            if (sd_is_socket_inet(SD_LISTEN_FDS_START + i, AF_INET6, SOCK_STREAM, 1, port) > 0) {
                fd = SD_LISTEN_FDS_START + i;
                activated = true;
                pa_log_info("socket(): Found socket activation socket for ipv6 in port '%d' \\o/", port);
                break;
            }
        }
    }
