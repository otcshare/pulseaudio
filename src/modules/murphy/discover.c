...
void pa_discover_register_sink_input(struct userdata *u, pa_sink_input *sinp)
{
     ...
     * route by specifying the target sink @ stream creation time.
     *
                                     ...
                                     pa_sink_input_new_data *data)
{
    ...
    if (pa_streq(mnam, "module-combine-sink")) {
        ...
        if (loopback) {
            ...
            pa_log_info("in discover calling to classify guess stream type");
    ...
    if (!data->sink) {
        ...
        if (sink) {
            ...
            if (pa_sink_input_new_data_set_sink(data, sink, false, false))
                                        ...
                                        pa_source_output_new_data *data)
{
    ...
    if (!data->source) {
        ...
        if (source) {
            if (pa_source_output_new_data_set_source(data, false, source, false)) {
