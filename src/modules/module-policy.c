  ...
  switch (command) {
    ...
    case SUBCOMMAND_BALANCE: {
        ...
        PA_IDXSET_FOREACH(s, u->core->sinks, idx) {
            cvol_p = pa_sink_get_volume (s, false);
            pa_cvolume_set_balance ((pa_cvolume *)cvol_p, &s->channel_map, u->balance);
            pa_sink_set_volume(s, (pa_cvolume *)cvol_p, true, true);
        }
        break;
    }
...
static pa_hook_result_t sink_input_new_hook_callback(pa_core *c, pa_sink_input_new_data *new_data, struct userdata *u)
{
    ...
    new_sink = policy_select_proper_sink(c, policy, u->is_mono);
...
static pa_hook_result_t sink_put_hook_callback(pa_core *c, pa_sink *sink, struct userdata *u)
{
    ...
    cvol = pa_sink_get_volume(sink, false);
    pa_cvolume_set_balance((pa_cvolume *)cvol, &sink->channel_map, u->balance);
    pa_sink_set_volume(sink, (pa_cvolume *)cvol, true, true);
...
static pa_hook_result_t source_output_new_hook_callback(pa_core *c, pa_source_output_new_data *new_data, struct userdata *u) {
    ...
    new_source = policy_select_proper_source(c, policy);
