...
Patch2:     set-input-source-manually.patch
Requires:   udev 
...
%package systemd
Summary:    Systemd configuration scripts  
Group:      Multimedia/PulseAudio  
Requires:   %{name} = %{version}-%{release}  
...
%reconfigure --disable-static --enable-alsa --disable-ipv6 --disable-oss-output --disable-oss-wrapper --enable-dlog --enable-bluez --disable-x11 --disable-hal --disable-hal-compat
make %{?jobs:-j%jobs}
...
install -m 644 %{SOURCE1}         %{buildroot}/%{_lib}/systemd/system/pulseaudio.service
